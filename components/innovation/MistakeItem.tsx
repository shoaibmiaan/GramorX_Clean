  ...
  return (
    <div className="p-3 rounded border flex items-start justify-between gap-3">
      <div>
        ...
        <div className="text-small text-muted-foreground mt-1">{item.excerpt}</div>
        <div className="text-caption text-muted-foreground mt-2">{new Date(item.created_at ?? Date.now()).toLocaleString()}</div>
      </div>
