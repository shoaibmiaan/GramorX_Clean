        ...
        <div className="animate-pulse space-y-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex gap-4">
              <div className="h-4 w-4 bg-lightBg dark:bg-dark/80 rounded-full"></div>
              <div className="flex-1 space-y-2">
                <div className="h-4 bg-lightBg dark:bg-dark/80 rounded w-3/4"></div>
                <div className="h-3 bg-lightBg dark:bg-dark/80 rounded w-1/2"></div>
              </div>
            </div>
          ))}
        </div>
  ...
  if (activities.length === 0) {
    return (
      <Card className="p-8 text-center">
        <Activity className="h-12 w-12 text-grayish mx-auto mb-4" />
        <h3 className="text-h4 font-semibold mb-2">No activities yet</h3>
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-h4 font-semibold">Recent Activities</h3>
        ...
        {activities.map((activity, index) => (
          <div key={activity.id} className="relative pl-8 pb-6 last:pb-0">
            ...
            {index < activities.length - 1 && (
              <div className="absolute left-[15px] top-[24px] bottom-0 w-0.5 bg-lightBg dark:bg-dark/80"></div>
            )}

            {/* Timeline dot */}
            <div className={`absolute left-0 top-1 w-7 h-7 rounded-full flex items-center justify-center ${
              getActivityColor(activity.activity_type) === 'blue' ? 'bg-electricBlue/15 dark:bg-electricBlue/95/30' :
              getActivityColor(activity.activity_type) === 'green' ? 'bg-success/15 dark:bg-success/95/30' :
              ...
              'bg-lightBg dark:bg-dark/90'
            }`}>
              <div className={`${
                getActivityColor(activity.activity_type) === 'blue' ? 'text-electricBlue dark:text-electricBlue/90' :
                getActivityColor(activity.activity_type) === 'green' ? 'text-success dark:text-success/90' :
                ...
                'text-grayish dark:text-grayish'
              }`}>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-small text-foreground mb-2">{activity.description}</p>

                {activity.metadata && Object.keys(activity.metadata).length > 0 && (
                  <div className="text-caption text-muted-foreground bg-lightCard dark:bg-dark/90/50 p-2 rounded">
