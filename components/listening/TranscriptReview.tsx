...
const TranscriptReview: React.FC<Props> = ({
    ...
    return sections
      .map((section) => ({
        ...
        questions: questionStatuses
          .filter(
            (status) => sectionAssignments.get(status.qno) === section.order
          )
  ...
  const hasTranscript = enrichedSections.some(
    (section) => (section.transcript ?? '').trim().length > 0
  );
    ...
    <Card className={`card-surface rounded-ds-2xl p-6 ${className}`}>
      <div className="flex flex-col gap-2">
        ...
        <p className="text-small text-muted-foreground">
          Review the audio transcripts and see which questions were associated
          with each section.
        </p>
      </div>
