  ...
  if (!feedback) {
    return (
      <Card className="space-y-2" padding="lg">
        <h2 className="text-h4 font-semibold text-foreground">Feedback</h2>
        <p className="text-small text-muted-foreground">Detailed feedback will appear once scoring completes.</p>
      </Card>
    );
  }
    ...
    <Card className="space-y-4" padding="lg">
      ...
      {feedback.highlights && feedback.highlights.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-small font-semibold uppercase tracking-wide text-muted-foreground">Highlights</h3>
      ...
      {feedback.fixes && feedback.fixes.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-small font-semibold uppercase tracking-wide text-muted-foreground">Fix this next</h3>
          <ul className="space-y-3">
            {feedback.fixes.map((fix) => (
              <li key={fix.title} className="rounded-2xl border border-border/60 bg-card/70 p-4">
                <p className="text-small font-semibold text-foreground">{fix.title}</p>
                <p className="text-small text-muted-foreground">{fix.why}</p>
                {fix.example && (
      ...
      {!feedback.highlights && (!feedback.fixes || feedback.fixes.length === 0) && (
        <p className="text-small text-muted-foreground">Keep practising—actionable feedback will appear soon.</p>
      )}
    </Card>
  );
};
