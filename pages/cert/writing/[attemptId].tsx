      ...
      <Container className="mx-auto max-w-3xl">
        <article className="rounded-3xl border border-border/40 bg-white p-10 shadow-2xl print:shadow-none">
          <header className="text-center">
            <p className="text-small uppercase tracking-[0.3em] text-muted-foreground">GramorX Writing Module</p>
            <h1 className="mt-4 text-display font-semibold text-foreground">Achievement Certificate</h1>
          ...
          <section className="mt-10 flex flex-col items-center justify-center gap-6 rounded-2xl border border-dashed border-primary/40 bg-primary/5 px-10 py-12 text-center">
            <span className="text-small font-medium uppercase tracking-[0.2em] text-primary/80">Overall band</span>
            ...
            {tasks.map((task) => (
              <div key={task.task} className="rounded-2xl border border-border/40 bg-muted/20 p-6">
                <div className="flex flex-wrap items-baseline justify-between gap-4">
                  <h2 className="text-h3 font-semibold text-foreground">Task {task.task.replace('task', '')}</h2>
                  <div className="flex items-baseline gap-2">
                    <span className="text-small uppercase tracking-[0.2em] text-muted-foreground">Band</span>
                    <span className="text-h1 font-semibold text-foreground">{task.band.toFixed(1)}</span>
                  </div>
                </div>
                <p className="mt-4 text-small leading-relaxed text-muted-foreground">{task.summary}</p>
              </div>
            ))}
          </section>

          <footer className="mt-12 border-t border-border/30 pt-6 text-small text-muted-foreground">
            <p>Attempt ID: {attemptId}</p>
            <p>Verified by the GramorX writing evaluation service.</p>
            <p className="mt-2 text-caption">For authenticity, cross-check this ID inside your GramorX dashboard.</p>
          </footer>
        </article>
      </Container>
    </div>
