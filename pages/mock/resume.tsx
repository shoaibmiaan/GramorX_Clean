...
const MockResumePage: NextPage & { getLayout?: (page: React.ReactNode) => React.ReactNode } = () => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-6 md:py-10">
      <div className="space-y-1">
        <p className="text-sm font-medium uppercase tracking-[0.08em] text-muted">Resume mock</p>
        <h1 className="text-2xl font-semibold text-foreground md:text-3xl">Pick up where you left</h1>
      </div>
        ...
        <div className="rounded-2xl border border-border bg-background/60 p-6">
          ...
          {canResume && (
            <div className="mt-6 rounded-xl border border-amber-300/40 bg-amber-500/10 p-4 text-sm text-amber-200">
              ...
              <p className="mt-1">
                We&apos;ll restore your timer and answers from this checkpoint. Before you resume, double-check your device, connection, and quiet environment so you don&apos;t lose time mid-section.
              </p>
            </div>
          )}
        </div>
...
MockResumePage.getLayout = (page: React.ReactNode) => <MockAllLayout>{page}</MockAllLayout>;

export default MockResumePage;
